...
//compile with gcc oMPNums.c  mmio.c -fopenmp -o openMPgccO4Allall.out -O4
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include "mmio.h"
...
int main(int argc, char *argv[]){
    ...
    for(test = 0 ; test <5 ; test++){
        ...
        //thread table with different values to benchmark the code

        int threadsTable[9] = {1,2,4,8,16,32,64,256,1024};    
        for(i = 0 ; i < 9 ; i++){
            for(int many = 0 ; many < 3 ; many++ ){
                for (uint32_t l = 0; l < cooWhole.n_nz ; l++) productValues[l] = 0;
                // variable to store the sum of the values of the product of the multiplication 
                uint32_t cumsum = 0;
                // start counting time 
                gettimeofday (&startwtime, NULL);
                // set the number of threads
                omp_set_num_threads(threadsTable[i]);
                // loop through the array of pointers and check if there is a non zero value in 
                //the same row as the column of the value currently checking (symmetric matrices with only 1 non zero values) 
                #pragma omp parallel for private(j,k,l) //reduction (+:cumsum)
                ...
                printf("%f,%u,%u,%d,%s\n",seq_time,cumsum, triangles, threadsTable[i],matrices[test]);
            }
        }
    }
}
