...
// clang OpenCilk.c mmio.c -O3 -o OpenCilk.out -fopencilk
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include "mmio.h"
...
Coo MtxToCoo(int argc,char *argv[]){
    ...
    if (argc < 2)
	{
		fprintf(stderr, "Usage: %s e to blaka\n", argv[0]);
		exit(1);
	}
    else    
    { 
...
int main(int argc, char *argv[]){
    ...
    Coo cooHalf = MtxToCoo(argc,argv);
    ...
    cilk_for(int g = 0 ; g < (cooWhole.n_cols + 1)  ; g++ ){
        for(int j = csrSparse.row_ptrs[g] ; j < csrSparse.row_ptrs[g+1] ; j++ ){
            for(int k = csrSparse.row_ptrs[csrSparse.col_indices[j]] ; k < csrSparse.row_ptrs[csrSparse.col_indices[j]+1] ; k++){
                for(int l = csrSparse.row_ptrs[g] ; l < csrSparse.row_ptrs[g+1] ; l++ ){
    ...
    cilk_for(int j = 0 ; j < cooWhole.n_nz ; j++){
    ...
    printf("%u cumulative sum, %u triangles \n",cumsum, triangles);
