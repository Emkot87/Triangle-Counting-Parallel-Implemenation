...
// gcc pThrds.c mmio.c -O4 -pthread -o pThrds.out
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include "mmio.h"
...
// Define the number of threads this will create
#define NTHREADS 8
...
void* Mults(void * arg){
    ...
    uint32_t start , end ;
    start = (csrSparse->iter)*((csrSparse->n_cols + 1)/NTHREADS) ;
    end = (csrSparse->iter + 1)*((csrSparse->n_cols + 1)/NTHREADS);
    
    
    if(csrSparse->iter == (NTHREADS - 1)){
...
int main(int argc, char *argv[]){ 
    ...
    for (i = 0; i < n; i++) {
        csrSparse[i].iter = i;
        pthread_create(&threads[i] ,NULL ,Mults ,(void *)(csrSparse+i));
    }
